...
describe Dessert do
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new('flan', 'lots', chef) }.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ['sugar', 'eggs', 'flour', 'water', 'butter']
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return(
        "Chef Ryan the Great Baker"
        )
      expect(cookie.serve).to eq(
        "Chef Ryan the Great Baker has made 50 cookies!"
      )
    end
  end
